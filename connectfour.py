...
class ConnectFourBoard(object):
    ...
    """
    # Connect-k component for the board #
    k = 20
    ...
    # passing additional k value in initializing the constructor of the board. Default value set to 4 #
    def __init__(self, board_array = None, board_already_won = None, modified_column = None, current_player = 1, previous_move = -1, k=20):
    ...
    # Return the value of k-connect component
    def get_k(self):
        return self.k
    ...
    def do_move(self, column):
        ...
        return ConnectFourBoard(new_board, board_already_won=self.is_win(), modified_column=column, current_player = self.get_other_player_id(), k=self.get_k())

    def _is_win_from_cell(self, row, col, k):
    ...
    # Check the win condition in Longest-Streak-To-Win Problem
    def _is_win_longest_streak(self, row, col,p):
        if turn is 20:
            ...
            if currId==1:
                ...
                otherId=1
            longestCurr=self.longest_chain(currId)
            longestOth=self.longest_chain(otherId)
            if longestCurr>longestOth:
                return currId
            elif longestOth > longestCurr:
                return otherId
            else:
                return 3
        else:
    ...
    def is_win(self):
        ...
        for i in xrange(self.board_height):
            for j in xrange(self.board_width):
                ...
                if cell_player != 0:
                    # Checking win condition in case of Longest-Streak-To-Win Problem
                    if self.get_k()==20:
                        win = self._is_win_longest_streak(i,j,cell_player)
                        if win==1:
                            return 1
                        elif win==2:
                            return 2
                        ...
                        win = self._is_win_from_cell(i,j,self.get_k())
                    if win:
                        self._is_win = win
                        return cell_player
    ...
    def clone(self):
        ...
        return ConnectFourBoard(self._board_array, board_already_won=self._is_win, current_player = self.get_current_player_id(), k=self.get_k())
...
class ConnectFourRunner(object):
    ...
    * The game ends when one of the two players has four consecutive tokens in a row
    ...
    def run_game(self, verbose=True):
        ...
        # Win_for_player=3 defines the Tie condition in case of Longest-Streak-To-Win Problem
        if win_for_player != 0 and self._board.is_tie() or win_for_player==3:
